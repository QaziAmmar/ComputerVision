...
def get_data(input_path):
    ...
    with open(input_path, 'r') as f:
        ...
        for line in f:
            ...
            # count the instance of each cell category.
...
# %% md #### Define ROI Pooling Convolutional Layer
...
# %% md #### Calculate IoU (Intersection of Union)
...
# %% md #### Calculate the rpn for all anchors of all images

def calc_rpn(C, img_data, width, height, resized_width, resized_height, img_length_calc_function):
    """(Important part!) Calculate the rpn for all anchors
        If feature map has shape 38x50=1900, there are 1900x9=17100 potential anchors

    Args: C: config img_data: augmented image data width: original image width (e.g. 600) height: original image
    height (e.g. 800) resized_width: resized image width according to C.im_size (e.g. 300) resized_height: resized
    image height according to C.im_size (e.g. 400) img_length_calc_function: function to calculate final layer's
    feature map (of base model) size according to input image size
                ...
                for jy in range(output_height):
                    ...
                    for bbox_num in range(num_bboxes):
                        ...
                        if curr_iou > best_iou_for_bbox[bbox_num] or curr_iou > C.rpn_max_overlap:
                            ...
                            # x,y are the center point of ground-truth bbox xa,ya are the center point of anchor bbox
                            # (xa=downscale * (ix + 0.5); ya=downscale * (iy+0.5)) w,h are the width and height of
                            # ground-truth bbox wa,ha are the width and height of anchor bboxe tx = (x - xa) / wa ty
                            # = (y - ya) / ha tw = log(w / wa) th = log(h / ha)
...
# %% md #### Get new image size and augment the image
...
def augment(img_data, config, augment=True):
    ...
    if augment:
        ...
        # Changing annotation according to flipping.
...
# %% md #### Generate the ground_truth anchors
