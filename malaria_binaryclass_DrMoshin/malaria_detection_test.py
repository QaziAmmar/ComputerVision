...
for i in range(len(basic_cnn_preds_labels)):
    ...
    if label == 'healthy':
        image_save_path = path.result_folder_path + "IML_dataset_trained_rbc/healthy/" + files_names[i]
        cv2.imwrite(image_save_path, img)
    else:
        image_save_path = path.result_folder_path + "IML_dataset_trained_rbc/malaria/" + files_names[i]
        cv2.imwrite(image_save_path, img)
